...
import { Button, MenuItem, Paper, Stack, TextField, Typography, Pagination } from '@mui/material';
...
export default function UsersPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      <Typography variant="h5" fontWeight={700} className="text-gray-800 dark:text-gray-100">
        {t('users')}
      </Typography>
      ...
      <Paper className="p-4 space-y-3 dark:bg-[#111827]">
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems="center">
          ...
          <TextField
            ...
            fullWidth
          />
          <TextField label={t('hasAvatar')} size="small" select value={hasAvatar} onChange={e=>setHasAvatar(e.target.value)}>
            <MenuItem value="any">{t('any')}</MenuItem>
            <MenuItem value="true">{t('yes')}</MenuItem>
            <MenuItem value="false">{t('no')}</MenuItem>
          </TextField>
          <TextField label={t('isDisabled')} size="small" select value={isDisabled} onChange={e=>setIsDisabled(e.target.value)}>
            <MenuItem value="any">{t('any')}</MenuItem>
            <MenuItem value="true">{t('yes')}</MenuItem>
            <MenuItem value="false">{t('no')}</MenuItem>
          </TextField>
          <Button variant="outlined" onClick={()=>refetch()}>{t('apply')}</Button>
        </Stack>
      </Paper>

      <Paper className="p-3 dark:bg-[#111827]">
        {isLoading ? (
          <div className="p-6 text-gray-700 dark:text-gray-300">{t('loading')}</div>
        ) : (
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {data?.data?.map(u => (
              <div key={u.id} className="flex items-center gap-3 border rounded-lg p-3 dark:border-gray-700">
                <img
                  src={u.avatarUrl ? u.avatarUrl : 'https://placehold.co/64x64?text=U'}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs uppercase mt-1 text-gray-600 dark:text-gray-300">{u.role}</div>
                </div>
              </div>
            ))}
          </div>
